...
class MHN():
	...
	def __init__(self, model_path, anchor_path='',
		  ...
		  labelmap= ['person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle', 'traffic light', 'traffic sign'],
		  cuda=False):
		self.labels = labelmap
		
		# Initialize model
		self.initialize_model(model_path, anchor_path, conf_thres, iou_thres, cuda)

	def __call__(self, image):
		return self.estimate_road(image)

	def initialize_model(self, model_path, anchor_path, conf_thres=0.3, iou_thres=0.5, cuda=False):
	...
	def draw_boxes(self, image, text=True):
		...
		return util_draw_detections(self.filtered_boxes, self.filtered_scores, self.filtered_indexes, image, self.labels, text)
